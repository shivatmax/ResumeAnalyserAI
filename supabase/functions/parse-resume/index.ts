  ...
  try {
    ...
    if (!unstructuredResponse.ok) {
      ...
      console.error('Unstructured API error:', {
        ...
        body: errorText,
        url: unstructuredResponse.url,
        headers: Object.fromEntries(unstructuredResponse.headers.entries()),
      });
      throw new Error(
        `Unstructured API error (${unstructuredResponse.status}): ${unstructuredResponse.statusText}. Details: ${errorText}`
      );
    }
    ...
    const processedText = parsedData.map((item: any) => item.text).join('\n');
